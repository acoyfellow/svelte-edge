...
export const AgentHome: FC = () => (
  <html lang="en">
    <head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>svelte-edge · agent code mode</title>
        ...
        <meta property="og:image" content="https://svelte-edge.coey.dev/og/index.png" />
        ...
        <meta name="twitter:image" content="https://svelte-edge.coey.dev/og/index.png" /><style>{html([styles] as unknown as TemplateStringsArray)}</style></head>
...
async function sendPrompt(prompt){ const model=document.getElementById('model-select').value; const modelName=document.getElementById('model-select').selectedOptions[0].textContent; addMsg('user','User',escapeHtml(prompt)); const thinking=addMsg('agent','Agent','Writing Svelte 5 with '+escapeHtml(modelName)+'…'); statusEl.textContent='Calling '+modelName+' via Workers AI…'; const t0=performance.now(); const res=await fetch('/agent/generate-ui',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({prompt,model})}); const data=await res.json(); const ms=Math.round(performance.now()-t0); if(!res.ok){thinking.querySelector('p').textContent='Generation failed. Try another model or prompt.'; statusEl.textContent='Generation failed'; return;} thinking.querySelector('p').textContent='Generated with '+(data.modelLabel||data.model)+' in '+(data.timings?.aiMs||ms)+'ms. I embedded it below.'; await compileAndRender(data.source);}
  ...
  return (
    <html lang="en">
      <head>
        ...
        <meta property="og:image" content="https://svelte-edge.coey.dev/og/index.png" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:image" content="https://svelte-edge.coey.dev/og/index.png" />
        <style>{styles}</style>
      </head>
...
async function generateUi() {
  ...
  const model = document.getElementById('model-select')?.value || 'fast';
  const res = await fetch('/agent/generate-ui', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt, model }) });
